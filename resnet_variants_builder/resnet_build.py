...
def _conv_bn_relu(**conv_params):
    ...
    def f(input):
        # Define an inner function "f" to return the function of "_bn_relu()"
    ...
    # Please note "f" as a returned value, which returns "_bn_relu(conv)"
    return f
...
def _residual_block(block_function, filters, repetitions, is_layer1=False):
    ...
    def f(input):
        for i in range(repetitions):
            ...
            input = block_function(filters=filters, init_strides=init_strides,
                                   is_block1_of_layer1=(is_layer1 and i==0))(input)
        return input

    return f


def basic_block(filters, init_strides=(1,1), is_block1_of_layer1=False):
    def f(input):
        if is_block1_of_layer1:
...
def bottleneck(filters, init_strides=(1,1), is_block1_of_layer1=False):
    # Return a final conv layer of filters*4
    def f(input):
        if is_block1_of_layer1:
            # Don't repeat bn->relu due to adoption of bn->relu->maxpool
...
def _handle_dim_ordering():
    ...
    if K.image_dim_ordering() == 'tf':
        ...
        CHANNEL_AXIS, ROW_AXIS, COL_AXIS = 1, 2, 3
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        # Build a custom ResNet-like network architecture.
        """
        Args:
            input_shape: The input shape in the form (nb_channels, nb_rows, nb_cols)
            num_outputs: The number of outputs at final softmax layer
            block_fn: Either `basic_block` or `bottleneck`, originally basic_block for layers < 50
        ...
        # Permute the dimension order if necessary
        if K.image_dim_ordering() == 'tf':
